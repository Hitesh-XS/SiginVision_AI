...
from vision.models import DynamicDatasetSample
from vision.services.landmark_utils import normalize_landmark_sequence


class Command(BaseCommand):
    help = "Train dynamic sign recognition model using landmark sequences"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="+",
            required=False,
            help="Dynamic classes to train, example: HELLO THANK_YOU HELP"
        )
    ...
    def handle(self, *args, **options):
        ...
        queryset = DynamicDatasetSample.objects.select_related("gesture").all()
        ...
        for sample in queryset:
            ...
            if len(sample.frames) != frame_count:
                continue
            ...
            normalized_sequence = normalize_landmark_sequence(sample.frames)

            if len(normalized_sequence) != frame_count:
                continue
        ...
        if len(X) == 0:
            self.stderr.write(self.style.ERROR("No valid dynamic samples found."))
            return
        ...
        self.stdout.write(self.style.NOTICE(f"Total dynamic samples: {len(X)}"))
        self.stdout.write(self.style.NOTICE(f"Input shape: {X.shape}"))
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.Input(shape=(frame_count, 63)),
        ...
        model_path = output_dir / "dynamic_gesture_model.keras"
        label_map_path = output_dir / "dynamic_label_map.json"
        report_path = output_dir / "dynamic_training_report.json"
